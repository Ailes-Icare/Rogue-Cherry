...
export default function CodeEditor({ 
    ...
    if (globalSearchBarText && globalSearchMarks.length > 0 && globalSearchOccIndex >= 0) {
      ...
      if (activeMark) {
        ...
        if (isEditable && textareaRef.current) {
          ...
          if (wrapper) {
            const lineHeight = 1.5 * fontSize;
            const targetScrollTop = lineIndex * lineHeight - (wrapper.clientHeight / 2) + (lineHeight / 2);
  ...
  // Initialize or update windowing when toggling invisibles or layout changes
  useEffect(() => {
  ...
  }, [showInvisibles, fontSize, isEditable, text]);
  ...
  useEffect(() => {
    if (triggerGoToLine) {
      ...
      if (isEditable) {
        ...
        if (wrapper) {
          const lineHeight = 1.5 * fontSize;
          const targetScrollTop = targetLineIndex * lineHeight - (wrapper.clientHeight / 2) + (lineHeight / 2);
          wrapper.scrollTo({ top: targetScrollTop, behavior: 'smooth' });
        }
      }
    }
  }, [triggerGoToLine, isEditable, fontSize]);

  const skipNextScrollRef = useRef(false);

  // Expose onToggleOccurrence to the global window for the memoized lines
  useEffect(() => {
    window.handleToggleOcc = (index) => {
      skipNextScrollRef.current = true;
      if (onToggleOccurrence) onToggleOccurrence(index);
    };
  ...
  useEffect(() => {
    ...
    if (activeOccIndex !== -1 && marks.length > 0) {
  ...
  }, [activeOccIndex, marks, text, fontSize]);

  // Scroll ciblé arbitraire (ex: depuis l'historique)
  useEffect(() => {
    if (scrollTargetIndex !== null && !isEditable) {
      const targetIndex = (scrollTargetIndex && typeof scrollTargetIndex === 'object') ? scrollTargetIndex.index : scrollTargetIndex;
      if (targetIndex !== null && targetIndex !== undefined && targetIndex >= 0 && targetIndex <= text.length) {
        const prevText = text.substring(0, targetIndex);
        const lineIndex = prevText.split('\n').length - 1;
        const wrapper = editorWrapperRef.current;
        if (wrapper) {
          const lineHeight = 1.5 * fontSize;
  ...
  }, [scrollTargetIndex, text, isEditable, fontSize]);
    ...
    <div className="flex-1 flex flex-col border border-border-dark overflow-hidden bg-bg-dark rounded-sm">
          ...
          {globalSearchBarText && (
            <div className={`flex items-center gap-3 transition-opacity ${globalSearchSuspended ? 'opacity-50 grayscale pointer-events-none' : ''}`}>
              ...
              <div className="text-[15px] uppercase font-bold text-[#aaa] flex items-center gap-2">
                <span className="hidden sm:inline">Similitude :</span> 
      ...
      >
        {isEditable ? (
          ...
          // Mode Lecture Seule : Rendu mémoïsé par lignes ultra-performant avec Virtualisation
          <div 
            ref={containerRef}
          ...
          >
            <div 
              ...
              style={{ 
                paddingTop: `${Math.max(0, visibleRange[0]) * 1.5 * fontSize}px`, 
                paddingBottom: `${Math.max(0, lineRecords.length - 1 - Math.min(lineRecords.length - 1, visibleRange[1])) * 1.5 * fontSize}px` 
              }}
            >
              {lineRecords.length === 0 ? (
                ...
                lineRecords.slice(Math.max(0, visibleRange[0]), Math.min(lineRecords.length - 1, visibleRange[1]) + 1).map((line) => (
                  <CodeLine
                    key={line.number}
                    number={line.number}
                    text={line.text}
