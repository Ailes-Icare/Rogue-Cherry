...
export default function SidebarRight({
  ...
  const [historyHeight, setHistoryHeight] = useState(Math.max(150, Math.min(300, Math.floor(window.innerHeight * 0.35))));
  const beforeScrollRef = useZoomable(14);
  const afterScrollRef = useZoomable(14);
  ...
  const [navBtnWidth, setNavBtnWidth] = useState(80);
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(entries => {
      for (let entry of entries) {
        setNavBtnWidth(entry.contentRect.width);
      }
    });
    observer.observe(navBtnRef.current);
    return () => observer.disconnect();
  }, []);
  ...
  const handleHistoryResize = (newY) => {
    ...
    const height = newY - parentTop - 60;
  ...
  const isNarrow = safeWidth < 320;
  const availWidth = safeWidth - 20; // 20px pour p-2.5 (10px de chaque côté)
  const totalWidthForBars = availWidth - 8; // 8px de gap-2 entre les deux blocs

  // Seuil de basculement en titre vertical ("PROJET") : totalWidthForBars < 281px (soit width < 301px)
  const useVerticalTitle = totalWidthForBars < 281;

  let blueCardWidth, resetWidth;
  if (useVerticalTitle) {
    // Mode Butée Verticale (Titre vertical) : la Blue Card peut descendre jusqu'à 196px
    resetWidth = Math.max(42, totalWidthForBars - 196);
    blueCardWidth = totalWidthForBars - resetWidth;
  } else {
    // Mode Horizontal : la Blue Card a besoin de minimum 235px pour son titre sur 2 lignes
    const targetBlue = totalWidthForBars * 0.75;
    if (targetBlue >= 235) {
      // Zone standard : répartition 75% / 25%
      blueCardWidth = targetBlue;
      resetWidth = totalWidthForBars - targetBlue;
    } else {
      // Zone de butée horizontale : la Blue Card reste à 235px et le Reset rétrécit sous les 25%
      blueCardWidth = 235;
      resetWidth = totalWidthForBars - 235;
    }
  }
  ...
  return (
    <div 
      ...
      style={{ width: `${safeWidth}px`, flexShrink: 9999 }} 
    ...
    >
      ...
      <div className="flex gap-2 h-[72px] flex-shrink-0">
        ...
        <div 
          style={{ width: `${blueCardWidth}px` }}
          className="bg-[#007acc] rounded-sm flex flex-row overflow-hidden p-[6px] shadow-sm"
        >
          {/* Titre à gauche, centré dans son espace bleu */}
          <div className={`${useVerticalTitle ? 'min-w-[28px] max-w-[55px]' : 'w-[70px]'} flex items-center justify-center px-1 flex-shrink-0`}>
            {useVerticalTitle ? (
              <div className="flex flex-col items-center justify-center text-[10px] font-black text-white leading-none uppercase tracking-tighter">
                <span>P</span>
                <span>R</span>
                <span>O</span>
                <span>J</span>
              ...
              <div className="flex flex-col items-center justify-center text-[14px] font-extrabold text-white text-center leading-[1.1] tracking-wide">
                <span className="whitespace-nowrap">Gestion</span>
                <span className="whitespace-nowrap">de projet</span>
              </div>
            )}
          </div>
        ...
        <button
          ...
          className={`flex-shrink-0 bg-cherry-red hover:bg-cherry-red-hover font-bold border border-cherry-red flex flex-row justify-evenly items-center rounded-sm transition duration-150 text-white shadow-sm overflow-hidden ${isProjectEmpty ? 'opacity-30 cursor-not-allowed' : ''}`}
                ...
                return (
                  ...
                  >
                    ...
                    <td className="p-2 text-right pr-3 text-[#999] text-[10px] font-mono align-top whitespace-nowrap w-px">
                      ...
                      {rec.isIgnored && rec.type === 'replace' && !rec.isReapplied && (
                         <div className="flex gap-1.5 justify-end mt-1 text-base">
                      ...
                      {rec.isIgnored && rec.type === 'replace' && rec.isReapplied && (
                         <div className="flex gap-1.5 justify-end mt-1 text-base">
                             <span title="Requête réappliquée avec succès" className="text-green-500 cursor-default">✔️</span>
                         </div>
                      )}
                    </td>
                  </tr>
      ...
      <div className="flex flex-col gap-1.5 flex-shrink-0 mt-1 select-none">
        ...
        <div className="flex flex-col">
          <div className="flex justify-between items-center mb-0.5">
            <span className="text-xs text-[#aaa] font-bold uppercase">Commentaire de l'action</span>
          ...
          <textarea
            ...
            className="w-full h-[35px] bg-bg-dark border border-border-dark text-[#d4d4d4] p-1.5 rounded-sm resize-none focus:outline-none font-sans italic"
        ...
        <div className="flex justify-between items-center text-xs gap-2">
          ...
          <div className="flex flex-1 gap-1 h-12 select-none">
            <div ref={navBtnRef} className="flex flex-col gap-0.5 w-10 h-12 flex-shrink-0 select-none">
              <button
                type="button"
                disabled={selectedIndex <= 0}
                onClick={() => onSelectIndex(selectedIndex - 1)}
                className={`flex-1 font-bold text-xs shadow-sm flex items-center justify-center rounded-sm ${selectedIndex <= 0 ? 'btn-disabled-unclickable' : 'btn-active'}`}
              ...
              <button
                ...
                onClick={() => onSelectIndex(selectedIndex + 1)}
                className={`flex-1 font-bold text-xs shadow-sm flex items-center justify-center rounded-sm ${(selectedIndex === -1 || selectedIndex === history.length - 1) ? 'btn-disabled-unclickable' : 'btn-active'}`}
          ...
          <div className="flex flex-col items-end gap-1.5 flex-shrink-0 ml-2">
            <span className="text-[10px] text-[#aaa] font-bold">
              {isNarrow ? "Détails" : "Détails (AVANT / APRÈS)"}
            </span>
            <div className="flex gap-2">
              <button
                ...
                className={`flex items-center justify-center rounded border transition ${
                  isNarrow ? 'w-[26px] h-[22px] text-[10px]' : 'w-8 h-7 text-base'
                } ${
                  isSyncScroll
              ...
              <button
                ...
                className={`border border-border-dark font-bold text-[#d4d4d4] flex items-center justify-center transition-all ${
                  isNarrow 
                    ? 'w-[26px] h-[22px] text-[10px] bg-[#2a2d2e] border-[#444]' 
                    : 'bg-bg-panel hover:bg-bg-dark px-3 py-1 rounded text-[11px]'
                }`}
                title={showMiniViews ? "Masquer les mini-views" : "Afficher les mini-views"}
              >
                {isNarrow ? (showMiniViews ? '▲' : '▼') : (showMiniViews ? 'Masquer ▲' : 'Afficher ▼')}
              </button>
            </div>
          </div>
        </div>
      ...
      {showMiniViews && (
        <div className="flex-1 flex flex-col gap-2 min-h-[100px] mt-1">
          ...
          <div className="flex-1 flex flex-col overflow-hidden min-h-0 bg-bg-dark border border-border-dark rounded-sm">
            <div className="bg-bg-panel px-2.5 py-0.5 text-xs text-[#888] font-bold border-b border-border-dark uppercase tracking-wider text-left">
              AVANT (FIND)
            </div>
            <div 
