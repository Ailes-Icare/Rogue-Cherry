...
export default function SmartDebugger({
  ...
  const zoomRefMirror = useZoomable(14); // Remplace mirrorScrollRef
  ...
  const handleUpdateHeader = (updates) => {
    ...
    const commentTag = newProps.comment ? `\n##Commentaire## ${newProps.comment}` : '';
    ...
    if (parsedRequest.isValid) {
      ...
      if (lines.length > 0 && lines[0].includes(syntaxConfig.START)) {
         lines[0] = header;
         if (lines.length > 1 && lines[1].startsWith('##Commentaire##')) {
            if (newProps.comment) lines[1] = `##Commentaire## ${newProps.comment}`;
            else lines.splice(1, 1);
         } else if (newProps.comment) {
            lines.splice(1, 0, `##Commentaire## ${newProps.comment}`);
         }
         setRawText(lines.join('\n'));
      } else {
         setRawText(`${header}${commentTag}\n${rawText}`);
      }
    }
  };
        ...
        <div className="p-3.5 bg-bg-dark border-t border-border-dark flex justify-between items-center gap-3 flex-shrink-0 select-none">
          ...
          <div className="flex flex-wrap items-center gap-3 w-full">
             <button
               ...
               className={`text-xs font-bold px-3 py-1.5 rounded transition ${parsedRequest.smartMode ? 'bg-[#4caf50] text-white shadow-[0_0_8px_rgba(76,175,80,0.6)]' : 'bg-[#555] text-[#ccc] hover:bg-[#666]'}`}
               title="Smart mode"
             >
               SMART
             </button>
             ...
             <div className="flex items-center gap-1">
               <button
                 ...
                 className={`text-xs font-bold px-3 py-1.5 rounded transition ${parsedRequest.multiMode ? 'bg-[#5c2d91] text-white shadow-[0_0_8px_rgba(92,45,145,0.6)]' : 'bg-[#555] text-[#ccc] hover:bg-[#666]'}`}
               >
                 MULTI
               </button>
               {parsedRequest.multiMode && (
                 <div className="flex items-center gap-2">
                   <input
                     ...
                     className="w-24 h-7 bg-[#1e1e1e] border border-border-dark text-[#ccc] text-xs px-2 rounded text-center"
                     placeholder="ex: 0,2,4"
                   />
                   <span className="text-[10px] text-[#888] max-w-[150px] leading-tight hidden xl:block">
                     * Indices depuis 0.
                   </span>
                 </div>
               )}
             ...
             <div className="flex items-center gap-2 ml-2 flex-1 min-w-[150px]">
               <span className="text-xs text-[#888] font-bold uppercase hidden md:inline">Label :</span>
               <span className="text-xs text-[#888] font-bold uppercase md:hidden">LBL :</span>
               <input
                 type="text"
                 value={parsedRequest.label || ''}
                 onChange={(e) => handleUpdateHeader({ label: e.target.value })}
                 className="flex-1 w-full h-7 bg-[#1e1e1e] border border-border-dark text-[#ccc] text-xs px-2 rounded focus:border-primary-blue transition-colors"
                 placeholder="Label de la requête"
               />
             </div>

             <div className="flex items-center gap-2 flex-[2] min-w-[200px]">
               <span className="text-xs text-[#888] font-bold uppercase hidden lg:inline">Commentaire :</span>
               <span className="text-xs text-[#888] font-bold uppercase lg:hidden">CMT :</span>
               <input
                 type="text"
                 value={parsedRequest.comment || ''}
                 onChange={(e) => handleUpdateHeader({ comment: e.target.value })}
                 className="flex-1 w-full h-7 bg-[#1e1e1e] border border-border-dark text-[#ccc] text-xs px-2 rounded focus:border-primary-blue transition-colors"
                 style={{ fontSize: 'calc(12px + var(--global-zoom-offset, 0px))' }}
