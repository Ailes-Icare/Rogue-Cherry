...
export default function App() {
  ...
  // États de Préférences par Défaut de l'Utilisateur
  const [userDefaultMulti, setUserDefaultMulti] = useState(() => localStorage.getItem('userDefaultMulti') === 'true');
  const [userDefaultSmart, setUserDefaultSmart] = useState(() => localStorage.getItem('userDefaultSmart') === 'true');

  // 2. États de Recherche et de Remplacements unitaire / multiple
  const [findText, setFindText] = useState("");
  const [replaceText, setReplaceText] = useState("");
  const [splitChars, setSplitChars] = useState(false);
  const [ignoreSpaces, setIgnoreSpaces] = useState(userDefaultSmart);
  const [multiMode, setMultiMode] = useState(userDefaultMulti);
  ...
  const [isCurrentRequestSyntax, setIsCurrentRequestSyntax] = useState(false);
  ...
  // ResizeObserver pour l'en-tête central
  ...
  const estimatedTitleWidth = (15 + (store.projectName || 'SANS_NOM').length + (store.currentVersion || '').length + 3) * 14.5;
  const estimatedTitleWidthOnTwoLines = (15 + (store.projectName || 'SANS_NOM').length) * 14.5;
  const scalingStartWidth = estimatedTitleWidthOnTwoLines + 520;

  let scale = 1.0;
  if (headerWidth < scalingStartWidth) {
    scale = Math.max(0.5, Math.min(1.0, headerWidth / scalingStartWidth));
  }

  const barHeight = Math.max(48, Math.min(95, 95 * scale));
  const isTitleMultiline = (headerWidth < estimatedTitleWidth + 520) || (headerWidth < 950) || (estimatedTitleWidth > 600);
  ...
  const [reloadedRecordIndex, setReloadedRecordIndex] = useState(null);
  ...
  // Zoom de police relatif (s'applique globalement à toutes les zones de texte)
  const [globalZoomOffset, setGlobalZoomOffset] = useState(0);
  ...
  useEffect(() => {
    const handleGlobalWheel = (e) => {
      if (e.ctrlKey) {
        ...
        setGlobalZoomOffset(prev => Math.max(-6, Math.min(26, prev + delta)));
      }
    };
  ...
  const handleImportMainCode = () => {
    navigator.clipboard.readText()
      .then(async clipText => {
        ...
        setMultiIndices([]);
        setMultiMode(false);
        setIsCurrentRequestSyntax(false);
  ...
  const handleGoToRecord = async () => {
    if (selectedRecordMarks && selectedRecordMarks.length > 0) {
      ...
      setHistoryScrollTarget({ index: selectedRecordMarks[0].start, timestamp: Date.now() });
    } else {
      await showAlert("Impossible de localiser cette modification dans le code actuel.", "Erreur");
    }
  };

  const handleGoToRecordForIndex = (idx) => {
    if (idx <= 0) return;
    const currentRec = store.history[idx];
    if (!currentRec || currentRec.type !== 'replace') return;

    const virtualTextBefore = rebuildTextAt(store.history, idx - 1, store.activeFileId);
    const virtualTextAfter = rebuildTextAt(store.history, idx, store.activeFileId);

    const { marksT2 } = computeLiveDiff(virtualTextBefore, virtualTextAfter, currentRec.splitChars);
    if (marksT2 && marksT2.length > 0) {
      setHistoryScrollTarget({ index: marksT2[0].start, timestamp: Date.now() });
    }
  };
  ...
  const handleReplace = async () => {
    ...
    // Sécurisation : Modification depuis une ancienne version
    if (store.selectedIndex < store.history.length - 1) {
      const choice = await showCustom({
        type: 'confirm',
        title: "Modification d'une version antérieure",
        message: "Vous êtes sur le point d'appliquer une modification sur une ancienne version de l'historique. Cela créera une incohérence (ou perdra le futur de l'historique si on force l'application).\n\nQue souhaitez-vous faire ?",
        buttons: [
          { label: 'Annuler l\'action', value: 'cancel', variant: 'secondary' },
          { label: 'Créer une nouvelle branche à partir d\'ici', value: 'branch', variant: 'primary' },
          { label: 'Revenir au dernier état et appliquer', value: 'latest', variant: 'primary' }
        ]
      });

      if (choice === 'cancel' || choice === null) {
        return; // On annule l'application
      } else if (choice === 'branch') {
        // On ouvre la modale de création de branche. L'utilisateur devra réappliquer ensuite.
        setVersionModalInitialTab('increment');
        setIsVersionModalOpen(true);
        return; 
      } else if (choice === 'latest') {
        const lastIdx = store.history.length - 1;
        store.setSelectedIndex(lastIdx);
        handleGoToRecordForIndex(lastIdx);
        // On laisse la fonction continuer pour s'appliquer sur la dernière version
      }
    }
    ...
    if (reloadedRecordIndex !== null && store.history[reloadedRecordIndex]) {
      const rec = store.history[reloadedRecordIndex];
      // Compare fields strictly to determine if it's the exact same request
      if (
        rec.findStr === findText &&
        rec.replaceStr === replaceText &&
        rec.multiMode === multiMode &&
        JSON.stringify(rec.multiIndices || []) === JSON.stringify(multiIndices || []) &&
        rec.ignoreSpaces === ignoreSpaces
      ) {
        store.markRecordAsReapplied(reloadedRecordIndex);
      }
    }
    setReloadedRecordIndex(null);

    // --- Calcul du nouveau texte pour les validations futures ---
    ...
    store.pushReplace(
      ...
      commentText || null,
      isCurrentRequestSyntax ? "requête" : "user"
    );
    ...
    if (pendingRequests.length > 0 && activeStackIndex >= 0) {
      ...
      if (nextIndex !== -1) {
        ...
        loadRequestIntoUI(newReqs[nextIndex].parsed, nextIndex, newReqs, null, newText);
        ...
        setPendingRequests([]);
        setActiveStackIndex(-1);
        setIgnoreSpaces(userDefaultSmart);
        setMultiMode(userDefaultMulti);
        setIsCurrentRequestSyntax(false);
      } else {
        setIsPlayingStack(false);
        setForcePlayStep(-1);
  ...
  const loadRequestIntoUI = async (parsed, index = activeStackIndex, currentReqs = pendingRequests, position = null, latestText = store.currentText) => {
    isUserEditingRef.current = false;
    if (parsed.isValid) {
      setIsCurrentRequestSyntax(true);
      if (parsed.multiMode && parsed.multiIndices && parsed.multiIndices.length === 0 && !parsed.multiExclude) {
        skipMultiResetRef.current = false;
      } else {
        skipMultiResetRef.current = true;
      }
      let finalMultiIndices = resolveMultiIndices(parsed, latestText);
      ...
      // Calcul synchrone explicite du scroll ciblé pour la première occurrence
      if (parsed.findText) {
        let firstOccStart = -1;
        if (parsed.smartMode) {
          const parts = parsed.findText.split(/\s+/).filter(p => p.length > 0);
          if (parts.length > 0) {
            const escapedParts = parts.map(p => p.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
            const regex = new RegExp(escapedParts.join('\\s+'), 'gi');
            const match = regex.exec(latestText);
            if (match) {
              firstOccStart = match.index;
            }
          }
        } else {
          firstOccStart = latestText.indexOf(parsed.findText);
        }
        if (firstOccStart !== -1) {
          setHistoryScrollTarget({ index: firstOccStart, timestamp: Date.now() });
        }
      }

      // Si erreur de localisation avec le vrai texte du store, on passe en error
      if (!parsed.smartMode) {
        const testOccs = latestText.indexOf(parsed.findText);
  ...
  const handleImportSyntaxRequest = async (clipboardText, position = null, isSmartCancelImport = false) => {
    if (!isSmartCancelImport) {
       cancelActiveSmartCancel();
       // Forcer le retour au dernier élément de l'historique au chargement d'une requête classique
       const lastIdx = store.history.length - 1;
       if (store.selectedIndex < lastIdx) {
         store.setSelectedIndex(lastIdx);
         handleGoToRecordForIndex(lastIdx);
       }
    }
  ...
  const handleCopyStack = async () => {
    ...
    const serialized = pendingRequests.map(req => {
      const p = req.parsed;
      const multiTag = p.multiMode 
        ? (p.multiIndices && p.multiIndices.length > 0 
           ? `[MULTI:${p.multiExclude ? 'NO ' : ''}${p.multiIndices.join(',')}]` 
           : '[MULTI:TRUE]') 
        : '';
      const smartTag = p.smartMode ? '[SMART:TRUE]' : '';
      const labelTag = p.label ? `[LABEL:${p.label}]` : '';
      
      const header = `${syntaxConfig.START} ${labelTag} ${multiTag} ${smartTag}`.replace(/\s+/g, ' ').trim();
      
      let commentPart = "";
      if (p.comment) {
        commentPart = `##Commentaire## ${p.comment}\n`;
      }
    ...
    try {
      ...
      await showAlert("Pile Multistack reconstituée copiée dans le presse-papier !");
    } catch (e) {
      await showAlert("Erreur lors de la copie de la pile.", "Erreur");
    }
  };
  ...
  const handleResizeLeft = (clientX) => {
    const maxLeftWidth = window.innerWidth - rightWidth - 450;
    if (clientX > 280 && clientX < maxLeftWidth) {
  ...
  const handleResizeRight = (clientX) => {
    ...
    const maxRightWidth = window.innerWidth - leftWidth - 450;
    if (w > 280 && w < maxRightWidth) {
      setRightWidth(w);
    }
  };
  ...
  const handleClearCurrentRequest = () => {
    isUserEditingRef.current = true;
    setIsCurrentRequestSyntax(false);
    setFindText("");
    setReplaceText("");
  ...
  const handleDoubleClickMulti = () => {
    const nextDefault = !userDefaultMulti;
    setUserDefaultMulti(nextDefault);
    localStorage.setItem('userDefaultMulti', String(nextDefault));
    setMultiMode(nextDefault);
  };

  const handleDoubleClickSmart = () => {
    const nextDefault = !userDefaultSmart;
    setUserDefaultSmart(nextDefault);
    localStorage.setItem('userDefaultSmart', String(nextDefault));
    setIgnoreSpaces(nextDefault);
  };

  return (
    <div 
      className="w-screen h-screen flex bg-bg-dark overflow-hidden font-segoe select-none relative"
      style={{ '--global-zoom-offset': `${globalZoomOffset}px` }}
    ...
    >
      ...
      {!isLeftCollapsed && (
        <SidebarLeft
          ...
          onClear={handleClearCurrentRequest}
          ...
          onDoubleClickMulti={handleDoubleClickMulti}
          onDoubleClickSmart={handleDoubleClickSmart}
        />
      )}
      ...
      <div className="flex-1 flex flex-col min-w-[450px] p-2.5 gap-2.5 overflow-hidden">
        
        {/* En-tête Éditeur Central */}
        <div 
          ref={headerRef}
          style={{ height: `${barHeight}px` }}
          className="flex justify-between items-center gap-4 border-b border-border-dark pb-2.5 flex-shrink-0 overflow-hidden"
        >
          <div 
          ...
          >
            {isTitleMultiline ? (
              <div 
                className="select-none font-bold mt-1 cursor-pointer hover:opacity-80 transition-opacity flex flex-col justify-center min-w-0" 
              ...
              >
                <div className="truncate text-left">
                  TEXTE SOURCE - <span className="text-[#FF4500] uppercase font-mono">{store.projectName || 'SANS_NOM'}</span>
                </div>
                <div className="text-left mt-0.5">
                  <span className="text-hl-yellow font-black font-mono" style={{ fontSize: `${30 * scale}px` }}>{store.currentVersion}</span>
                </div>
              </div>
            ) : (
              <div 
                className="select-none font-bold mt-1 truncate cursor-pointer hover:opacity-80 transition-opacity text-left min-w-0" 
                title={`Double-clic pour renommer le projet. TEXTE SOURCE - ${store.projectName || 'SANS_NOM'} - ${store.currentVersion}`}
                onDoubleClick={() => {
                  setVersionModalInitialTab('rename');
                  setIsVersionModalOpen(true);
              ...
              >
                TEXTE SOURCE - <span className="text-[#FF4500] uppercase font-mono">{store.projectName || 'SANS_NOM'}</span> - <span className="text-hl-yellow font-black font-mono" style={{ fontSize: `${30 * scale}px` }}>{store.currentVersion}</span>
              </div>
            )}
          </div>
          ...
          {(() => {
            const showButtonLabels = scale >= 0.8;
            const svgSize = showButtonLabels ? 24 : 32;
            return (
              <div 
                style={{
                  width: `${520 * scale}px`,
              ...
              >
                <div 
                  className="flex gap-2 items-stretch h-[70px]"
                  style={{
                ...
                >
                  <button
                    ...
                    className={`w-[72px] flex-shrink-0 rounded flex flex-col justify-center items-center gap-1 transition text-xs font-bold hover:border-[#20b2aa] ${
                      !store.currentText ? 'btn-active-blue' : 'btn-active'
                    }`}
                    title="Ouvrir un fichier"
                  >
                    <svg width={svgSize} height={svgSize} viewBox="0 0 24 24" fill="none" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  <button
                    ...
                    className={`w-[72px] flex-shrink-0 rounded flex flex-col justify-center items-center gap-1 transition text-xs font-bold hover:border-[#4da6ff] ${
                      !store.currentText ? 'btn-active-blue' : 'btn-active'
                    }`}
                  ...
                  <button
                    ...
                    className={`w-[72px] flex-shrink-0 rounded flex flex-col justify-center items-center gap-1 transition text-xs font-bold hover:border-primary-blue ${
                  ...
                  <button
                    ...
                    disabled={!store.currentText}
                    className={`w-[72px] flex-shrink-0 rounded flex flex-col justify-center items-center gap-1 transition text-xs font-bold hover:border-[#ffd700] ${
                      !store.currentText ? 'btn-disabled-unclickable' : 'btn-active'
                    } ${isProjectClean ? 'opacity-50' : ''}`}
                  ...
                  >
                    <svg width={svgSize} height={svgSize} viewBox="0 0 24 24" fill="none" stroke="#20b2aa" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  <button
                    ...
                    className={`w-[72px] flex-shrink-0 flex flex-col justify-center items-center rounded-sm transition gap-1 shadow-md hover:shadow-lg ${
                      !store.currentText 
                  ...
                  <div className="flex flex-col gap-1 w-10">
                    <button 
                      onClick={() => setGlobalZoomOffset(prev => Math.min(26, prev + 1))}
                    ...
                    <button 
                      onClick={() => setGlobalZoomOffset(prev => Math.max(-6, prev - 1))}
                      className="flex-1 bg-bg-panel-light hover:border-primary-blue border border-border-dark rounded flex items-center justify-center text-sm font-bold text-white transition"
        ...
        <CodeEditor
          ...
          fontSize={codeEditorFontSize + globalZoomOffset}
          setFontSize={setCodeEditorFontSize}
          statusBarInfo={
            isGlobalSearching 
      ...
      {!isRightCollapsed && (
        ...
        onLoadRequestToSidebar={(index) => {
          ...
          setIsCurrentRequestSyntax(rec.source === "requête");
